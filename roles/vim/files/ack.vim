...
" customize ack
fun! SearchContentAndShowMatches(pattern, ...)
  let path = a:0 < 1 ? '' : a:1
  let search = substitute(a:pattern, "\'", "\\\\x27", "g")
  exe "Ack! '".search."' ".path
endfun
command! -nargs=* Search call SearchContentAndShowMatches(<f-args>)
...
fun! SearchContentAndShowFiles(pattern, ...)
  ...
  redraw!
endfun
command! -nargs=* Encounter call SearchContentAndShowFiles(<f-args>)
